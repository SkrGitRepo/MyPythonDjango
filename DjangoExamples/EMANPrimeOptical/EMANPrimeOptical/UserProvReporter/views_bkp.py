...
def search_user(request):
    ...
    if request.method=='POST':
        ...
        if 'userid' in request.GET and request.GET['userid']:
            ...
            if usertype == 'All' and lifecycle == 'prod':
                ...
                #onramp_provisioned_status = get_onramp_status_for_all_user_role(userid)
...
def cpo_prod_superuser(request):
    #cpo_user_list = USER_TABLE.objects.using('eon_rtp5_1_l').all()
    cpo_prod2_superuser_list = USER_TABLE.objects.using('eon_rtp5_1_l').filter(subtypeofuser= 1).order_by('userid')
    tmpl=loader.get_template("cpo_prod_superuser.html")
    ...
    #----------------------------------------------------------------------------------
    cpo_prod1_superuser_list = USER_TABLE.objects.using('eon_rch1_1_l').filter(subtypeofuser= 1).order_by('userid')
    
    cpo_prod1_superusers=[]
    
    for user in cpo_prod1_superuser_list:
        cpo_prod1_superusers.append(user.username)
